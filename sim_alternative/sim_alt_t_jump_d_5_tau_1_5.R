...
n_values = c(2000)
d_values = c(5)
rho1_values = c(0.25, 0.5, 0.75, 0.95)
nu_param = 5  #df t marginals

#tau and L_n types from the null hypothesis script
tau_values = c(-1.5)
L_n_types = c("0.1(log(n))^2")

#Implement the DGP
dgp_functions <- list(
  "t_Jump_Alternative" = function(n_val, d_val, rho1) {
    Gen.from.t.alt(n = n_val, d = d_val, scenario = "jump", rho1_jump = rho1, marginal_df = nu_param)
  }
)
...
# ─────────────────────────────────────────────
run_one <- function(n, d, dgp_name, dgp_functions, rho1, tau, L_n_type) {
  
  # Generate data from the specified DGP
  X_sim <- dgp_functions[[dgp_name]](n, d, rho1)
...
clusterExport(cl, c("dgp_functions", "run_one", "find_k_opt", 
                    "nu_param", "MC.Bootstrap", "Gen.from.clayton.alt", "Gen.from.t.alt", "CUSUM.sedcd", "get.teta.sedcd", "Heq.sedcd"))
...
write.csv(sim_results, "sim_alt_t_jump_d_5_tau_1_5.csv", row.names = FALSE)
cat("Simulation complete. Results saved to sim_alt_t_jump_d_5_tau_1_5.csv.
")
