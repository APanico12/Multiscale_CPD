# =============================================================================
# RobWBSV2.R: Wild Binary Segmentation using Paper CUSUM Test Statistic T_n(u)
#
# Implements:
#   - Linearized pseudo-observation generator \hat{Z}_t
#   - Exact paper CUSUM test statistic T_n(u) as contrast over random intervals:
#       T_n(u) = |(1 / sqrt(n)) * (S1 - u * S_total)|
...
#   - Publication-quality visualization with horizontal tiered date labels
# =============================================================================
...
# -----------------------------------------------------------------------------
# 2. CUSUM Contrast Calculation using Paper Test Statistic T_n(u)
# -----------------------------------------------------------------------------

#' Fast CUSUM contrast calculation using paper's exact test statistic T_n(u)
#'
#' Evaluates:
#'   If weighted = TRUE:
#'     T^*_n(u) = |(1 / sqrt(n)) * (S1 - u * S_total)| / sqrt(u * (1 - u))
#'   If weighted = FALSE:
#'     T_n(u)   = |(1 / sqrt(n)) * (S1 - u * S_total)|
#' where u = (b - s + 1) / n, S1 = sum_{t=s}^b Z_t, S_total = sum_{t=s}^e Z_t
#'
#' @param cum_y     numeric vector c(0, cumsum(y))
#' @param s         start index
#' @param e         end index
#' @param min_dist  minimum distance from boundaries
#' @param weighted  whether to divide by sqrt(u * (1 - u)) (default: TRUE)
#' @return list(max_contrast, best_b)
fast_cusum_contrast <- function(cum_y, s, e, min_dist = 5, weighted = TRUE) {
  n <- e - s + 1
  if (n < 2 * min_dist) {
  ...
  u <- (b_candidates - s + 1) / n

  # S1 = sum_{t=s}^b Z_t, S_total = sum_{t=s}^e Z_t
  S1      <- cum_y[b_candidates + 1] - cum_y[s]
  S_total <- cum_y[e + 1] - cum_y[s]

  # Paper CUSUM statistic:
  T_vals <- abs((1 / sqrt(n)) * (S1 - u * S_total))
  if (weighted) {
    T_vals <- T_vals / sqrt(u * (1 - u))
  }

  max_idx <- which.max(T_vals)
  best_contrast <- T_vals[max_idx]
  best_b        <- b_candidates[max_idx]

  return(list(
    max_contrast = best_contrast,
    best_b       = best_b
  ))
}
...
wbs_recursive <- function(s, e, cum_y, intervals, min_dist = 5, weighted = TRUE) {
  ...
  if (length(sub_idx) > 0) {
    for (m in sub_idx) {
      sm <- intervals[m, 1]
      em <- intervals[m, 2]
      res_m <- fast_cusum_contrast(cum_y, sm, em, min_dist = min_dist, weighted = weighted)
  ...
  res_full <- fast_cusum_contrast(cum_y, s, e, min_dist = min_dist, weighted = weighted)
  ...
  left_tree  <- if (best_b - s >= 2 * min_dist) wbs_recursive(s, best_b, cum_y, intervals, min_dist, weighted = weighted) else NULL
  right_tree <- if (e - best_b >= 2 * min_dist) wbs_recursive(best_b + 1, e, cum_y, intervals, min_dist, weighted = weighted) else NULL
...
#' Wild Binary Segmentation for Robust Location Changepoints (Version 2)
#'
...
#' @param weighted      whether to divide contrast by sqrt(u * (1 - u)) (default: TRUE)
...
#' @param cex_break     font size for changepoint date labels (default: 1.15)
#' @param date_format   date format string (default: "%Y-%m-%d")
#' @param min_label_gap fraction of label width for collision gap (default: 0.15)
...
WBS.mean <- function(x, M = 5000, Kmax = 20,
  ...
  if (linearized) {
    y_series <- get_linearized_series(x, k = k, lag = lag, c = c, loss = loss)
  ...
  # 3. Explore tree of candidate change-points using exact T_n(u) CUSUM statistic
  tree_res <- wbs_recursive(1, N, cum_y = cum_y, intervals = intervals, min_dist = min_dist, weighted = weighted)
...
# -----------------------------------------------------------------------------
# 6. Publication-Quality Visualization Function: plot_wbs
# -----------------------------------------------------------------------------
...
#' @param ylab           y-axis label
#' @param xlab           x-axis label (defaults to "Date" if dates provided, else "Time (t)")
#' @param flag_outliers  logical, detect and circle severe isolated outliers (default: TRUE)
#' @param outlier_thresh Hampel score threshold for outliers (default: 4.0)
#' @param level_col      color for estimated segment levels (default: "#1565c0")
#' @param level_lwd      line width for segment levels (default: 2.4)
#' @param break_col      color for detected changepoints (default: "#b71c1c")
#' @param break_lwd      line width for changepoints (default: 1.8)
#' @param break_lty      line type for changepoints (default: 2)
#' @param outlier_col    color for outlier points (default: "#c62828")
#' @param series_col     color for observed time series line (default: "gray25")
#' @param series_lwd     line width for observed series (default: 1.3)
#' @param cex_axis       axis tick label font size (default: 1.6)
#' @param cex_lab        axis title font size (default: 1.7)
#' @param cex_main       main title font size (default: 1.8)
#' @param cex_legend     legend font size (default: 1.3)
#' @param cex_break      font size for horizontal changepoint date labels (default: 1.15)
#' @param date_format    format for date labels (default: "%Y-%m-%d")
#' @param min_label_gap  fractional horizontal safety gap before staggering (default: 0.15)
...
plot_wbs <- function(wbs_res,
  ...
  if (is.null(main)) {
    main <- "Robust Linearized WBS Changepoint Detection (V2)"
  }
  ...
  draw_canvas <- function() {
    top_mar <- if (length(wbs_res$cpts) > 0) 6.8 else 3.2
