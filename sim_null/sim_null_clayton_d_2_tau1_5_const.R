...
# Source Dependences
# ─────────────────────────────────────────────
# Corrected paths to match the local worker node environment[cite: 1]
source("sedcd_try.R")
source("DGP.R")
...
dgp_functions <- list(
  ...
  "Clayton" = function(n_val, d_val) {
    Gen.from.clayton(n = n_val, d = d_val, theta = theta.Clayton,
...
# HPC-aware core detection correctly using SLURM_CPUS_PER_TASK[cite: 1]
cores_str <- Sys.getenv("SLURM_CPUS_PER_TASK")
if (nchar(cores_str) > 0) {
  cores <- as.integer(cores_str)
  cat("SLURM environment detected. Using allocated CPUs:", cores, "\n")
} else {
  cores <- 4
  cat("SLURM_CPUS_PER_TASK not found. Falling back to safe limit:", cores, "cores\n")
}
...
write.csv(sim_results, "sim_null_clayton_d_2_tau1_5_const.csv", row.names = FALSE)
cat("Simulation complete. Results saved to sim_null_clayton_d_2_tau1_5_const.csv.\n")